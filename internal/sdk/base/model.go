...
type (
	...
	TimeModel struct {
		CreateTime types.Timestamp `xorm:"created comment('创建时间')"` // 创建时间
		UpdateTime types.Timestamp `xorm:"updated comment('修改时间')"` // 更新时间
		DeleteTime types.Timestamp `xorm:"deleted comment('删除时间')"` // 删除时间
	}

	// ExtModel 通用扩展字段
	ExtModel struct {
		Ext types.Ext `xorm:"not null default '{}' JSONB"` // 扩展字段
	}
)
